...
const Home = () => {
	...
	const likePost = (id) => {
		axios.put(`http://54.180.20.161:5000/like`, { postId: id }, config)
			.then((result) => {
				const newData = result_data.map((item) => {
	...
	const unlikePost = (id) => {
		axios.put(`http://54.180.20.161:5000/Unlike`, { postId: id }, config)
			.then((res) => {
				const newData = result_data.map((item) => {
	...
	const bookmark = (id) => {
		axios.put(`http://54.180.20.161:5000/bookmark-post`, { postId: id }, config)
			.then((result) => {
				dispatch({
					type: BOOKMARK_POST,
	...
	const removeBookmark = (id) => {
		axios.put(`http://54.180.20.161:5000/remove-bookmark`, { postId: id }, config)
			.then((result) => {
				dispatch({
					type: BOOKMARK_POST,
					payload: { Bookmarks: result.data.Bookmarks },
	...
	const makeComment = (text, postId) => {
		setComment("");
		axios.put(`http://54.180.20.161:5000/comment`, { text, postId }, config)
			.then((result) => {
				const newData = result_data.map((item) => {
					if (result.data._id === item._id) return result.data;
					else return item;
	...
	const deletePost = (postId) => {
		axios.delete(`http://54.180.20.161:5000/deletepost/${postId}`, config).then((res) => {
			const newData = result_data.filter((item) => {
