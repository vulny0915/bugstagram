...
const UserProfilePage = () => {
	...
	useEffect(() => {
		axios.get(`http://54.180.20.161:5000/user/${userid}`, config).then((res) => {
			setData(res.data);
			if(res.data.user.Followers.includes(state.user._id)){
				setShowFollow(false)
			}
		});
	}, []);
	...
	const followUser = () => {
		axios.put(`http://54.180.20.161:5000/follow`, { followId: userid }, config).then((result) => {
			dispatch({
	...
	const unfollowUser = () => {
		axios.put(`http://54.180.20.161:5000/unfollow`, { unfollowId: userid }, config).then((result) => {
			dispatch({
				type: UPDATE_FOLLOW_DATA,
				payload: { Followers: result.data.Followers, Following: result.data.Following },
			});
